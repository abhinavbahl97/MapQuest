...
public class Treap<K, V> extends AbstractMap<K, V> implements
    ...
    public V remove(Object key) {
    	if(this.root == null || key == null || !this.containsKey(key))
    	{
    		return null;
    	}
    	else{
    		V val = this.get(key);
    		root = removeHelper(this.root, key);
    		this.size--;
    		return val;
    	}
    }

    private TreapNode<K, V> removeHelper(TreapNode<K, V> curr, Object key) {
    	if(curr == null){
			return null;
		}
		int compareVal = this.compare(key, curr.key);
    ...
    class EntrySet extends AbstractSet<Map.Entry<K, V>> {
        ...
        public boolean remove(Object o) {
            if(o instanceof Map.Entry)
            {
            	if(contains((Map.Entry<K, V>) o))
            	{
            		Treap.this.remove(((Map.Entry<K, V>) o).getKey());
            		return true;
            	}
            }
            else
    ...
    abstract class PrivateNodeIterator<T> implements Iterator<T> {
        ...
        public void remove() {
            ...
            checker = 0;
            Treap.this.remove(lastReturned.key);  
    ...
    final class SubMap<K, V> extends AbstractMap<K, V> implements
        ...
        class EntrySetView extends AbstractSet<Map.Entry<K, V>> {
                    ...
                    public void remove() {
                    	if(checker == 0)
                        {
                        	...
                        	checker = 0;              
                        	 ...
                        	 this.expectedModCount = Treap.this.modCount;
                        	}
                    }
                };
            }
            ...
            public boolean remove(Object o) {
                 	if (o instanceof Map.Entry) {
                 		Map.Entry<K, V> start = (Map.Entry<K, V>) o;
     					if((!inRange(start.getKey())))
     						return false;
     					else if(m.containsKey(start.getKey()) && 
     							m.get(start.getKey()).equals(
     							start.getValue())) 
     					{
     						 m.remove(start.getKey());
     						return true;
     					}
     				}
     				return false;
