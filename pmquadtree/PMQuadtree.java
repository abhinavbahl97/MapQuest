...
import cmsc420.command.RoadIntersectsException;
...
public abstract class PMQuadtree {
	...
	public abstract class Node {
		...
		public Node add(final Geometry g, final Point2D.Float origin,
				final int width, final int height) throws PMNotValid, InvalidPartitionException{
	...
	public class Black extends Node {
		...
		public Node add(final Geometry g, final Point2D.Float origin,
			...
			if(g.isAirport()) {
				Airport airport = (Airport) g;
				for (Geometry geo : geometry) {
					if(geo.isRoad()) {

							if (Inclusive2DIntersectionVerifier.intersects(airport.getLocalCoordinates(), ((Road) geo).toLine2D())) 
							{
		 ...
		 */
		private Node partition(final Point2D.Float origin, final int width, final int height) throws PMNotValid, InvalidPartitionException  
		{
			if(width<=1 || height <=1 )
			{
				throw new PMNotValid();
			}
			//][			
	...
	public void addRoad(final Road g) 
			throws RoadAlreadyExistsThrowable, OutOfBoundsThrowable,
			 InvalidPartitionException, PMNotValid, RoadIntersectsException 
	{
		...
		{
			if(Inclusive2DIntersectionVerifier.intersects(road.toLine2D(), g.toLine2D()))
			{
				if(!road.contains(g.getStart()) && !road.contains(g.getEnd()))
				{
					throw new RoadIntersectsException();
				}
			}
		}
	...
	public void addAirport(final Airport airport) throws OutOfBoundsThrowable, InvalidPartitionException, RoadIntersectsException 
	{

		if (!Inclusive2DIntersectionVerifier.intersects(airport.getLocalCoordinates(),
	...
	public void addIsolatedCity(final City c) 
			throws IsolatedCityAlreadyExistsThrowable, RoadAlreadyExistsThrowable, OutOfBoundsThrowable, PMNotValid, InvalidPartitionException {
	...
	public void invalidHelper(Node node)
	{
		this.root = node;
	}
	
	public Node invalidPMCopy(Node node)
	{
		if(node.type== 0)
		{
			White whiteNode = new White();
			return whiteNode;
		}
		else if (node.type == 1)
		{
			Black blackNode = new Black();
			Black blackNode2 = (Black) node;
			blackNode.numPoints = blackNode2.numPoints;
			LinkedList<Geometry> geo = new LinkedList<Geometry>();
			for (Geometry geom : blackNode2.geometry) 
			{
				geo.add(geom);
			}
			blackNode.geometry = geo;
			return blackNode;
			
		}
		else
		{
			Gray gray2 = (Gray) node;
			Gray gray = new Gray(gray2.origin, 2*gray2.halfWidth, 2*gray2.halfHeight);
			for(int i=0 ; i<4 ; i++)
			{
				gray.children[i] = invalidPMCopy(gray2.getChild(i));
			}
			return gray;
		}
	}
